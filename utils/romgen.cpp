...
int main(int argc, char* argv[])
{
	...
	if(argc < 3)
	{
		cerr << "\nUsage: romgen <input file> <entity name> \n";
...
//		cerr << "\n";
		...
		cerr << "  a  - rtl model rom array (standard)\n";
...
//		cerr << "  d  - Xilinx distributed ram [not supported yet]\n";
		...
		cerr << "for example romgen fred.bin fred_rom c r\n\n";
		return -1;
	}
	...
	char option_1 = 0;
	char option_2 = 0;
	if (argc > 3) sscanf(argv[3],"%c",&rom_type);
	if (argc > 4) sscanf(argv[4],"%c",&option_1);
	if (argc > 5) sscanf(argv[5],"%c",&option_2);
